...
int main()
{
    ...
    if (!paddleSoundBuffer.loadFromFile("resources/paddle_hit.wav"))
        return EXIT_FAILURE;
    sf::Sound paddleSound(paddleSoundBuffer);

    sf::SoundBuffer wallSoundBuffer;
    if(!wallSoundBuffer.loadFromFile("resources/wall_hit.wav"))
        return EXIT_FAILURE;
    sf::Sound wallSound(wallSoundBuffer);
    ...
    // Load the text font
    sf::Font font;
    if (!font.loadFromFile("resources/NES-Chimera.otf"))
        return EXIT_FAILURE;
    ...
    scoreText.setPosition(gameWidth / 2, gameHeight / 4);
    scoreText.setFillColor(sf::Color::White);


    // Define the paddles properties
    sf::Clock AITimer;
    const sf::Time AITime   = sf::seconds(0.f);
    ...
    while (window.isOpen())
    {
        ...
        if (isPlaying)
        {
            ...
            {
                int rands = random(0, 500);
                
                AITimer.restart();
                if (ball.getPosition().y + ballRadius > rightPaddle.getPosition().y + paddleSize.y / 2)
                {
                    rightPaddleSpeed = paddleSpeed - rands;
